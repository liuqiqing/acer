...
// 获取当天的一窗收件数量
    ...
    getckxxqkss();
    //排队等待
     getpdddzzt(dateFormat("YYYY-mm-dd", date), "", "", "15");
    //省收件
    getssj(dateFormat("YYYY-mm-dd", date), "", "", "15");
    //空闲窗口统计
    getckkxck(dateFormat("YYYY-mm-dd", date), "", "", "15");
    ...
    $("body").on(
        ...
        function () {
            ...
            switch (target) {
                ...
                case 'brbjProvidebox':
                    day = dateFormat("YYYY-mm-dd", date);
                    getbrbjlb(day, "", "", "");// 本日办结数据列表
                    break;

                case 'brslProvidebox':
                    day = dateFormat("YYYY-mm-dd", date);
                    getbrsllb(day, "", "", "");// 本日受理数据列表
                    break;
                case 'xxResourcebox':
                    day = dateFormat("YYYY-mm-dd", date);
                    getkxcklb(day, "", "", "");// 空闲窗口数据列表
                    break;
            }
...
function getycsjsl() {
    ...
    $.ajax({
        ...
        success: function (result) {
            console.log(result)
            $("#total_count").html(result.sjsum);
            // 省收件数
            $("#sum_success_times").html(result.sycsum);
            // 市收件数
...
// 获取排队等待详细情况
function getpdddzzt(day, mounth, year, page) {
    var getpdddck = ctx + "/sxgl/getpdddck";
    var arr = new Array();
    $.ajax({
        type: "get",
        url: getpdddck,
        ...
        success: function (result) {
            ...
            for (var i = 0; i < result.pdddinfor.length; i++) {
                arr.push(result.pdddinfor[i].code_type);
                arrs.push(result.pdddinfor[i].sum1)
            }
            ;
            // 获取图表 如果page是空的，说明要获取全部的部门数量。如果不是空的默认展示15条数据。
            if (page != "") {
                // getchartssss(arr, arrs);
            } else {
                //如果page是空的，说明要获取全部的部门数量，并生成列表
                var applyHtml = "";
                for (var i = 0; i < result.pdddinfor.length; i++) {
                    applyHtml += "<tr>";
                    applyHtml += "<td>" + (i + 1) + "</td>";
                    applyHtml += "<td class='shqshjResourceBM'>" + result.pdddinfor[i].code_type + "</td>"; // 部门名称
                    applyHtml += "<td>" + result.pdddinfor[i].sum1 + "</td>"; // 被申请数（项）
                    applyHtml += "</tr>";
                }
                // htmlElement =applyHtml;
                $("#pdddxhProvideProp tbody").html(applyHtml);
            }
        },
        error: function (error) {
        }
...
// 排队等待：下载
function pdddProvideDownload() {
    var target = $("#pdddProvide_propTab ul li[class='active']").attr("data-target");
    var date = new Date();
    switch (target) {
        case 'day':
            window.location.href = ctx + "/sxgl/downloadpddd?year=&month=&day=" + dateFormat("YYYY-mm-dd", date);
            break;
        case 'month':
            window.location.href = ctx + "/sxgl/downloadpddd?year=&month=" + dateFormat("YYYY-mm", date) + "&day=";
            break;
        case 'year':
            window.location.href = ctx + "/sxgl/downloadpddd?year=" + dateFormat("YYYY", date) + "&month=&day=";
            break;
    }
}
...
//排队等待窗口年月日窗口切换
$("#pdddProvide_propTab ul li").click(function () {
    var date = new Date();
    $(this).addClass('active').siblings().removeClass('active');
    if ($(this).html() == '当日') {
        day = dateFormat("YYYY-mm-dd", date);
...
// 获取省收件详细情况
function getssj(day, mounth, year, page) {
    var getssj = ctx + "/tyslcontroller/getssjph";
    var arr = new Array();
    $.ajax({
        type: "get",
        url: getssj,
        ...
        success: function (result) {
            ...
            for (var i = 0; i < result.ssjph.length; i++) {
                arr.push(result.ssjph[i].item_name);
                arrs.push(result.ssjph[i].type);
                arrs.push(result.ssjph[i].region_name);
                arrs.push(result.ssjph[i].applicant);
                arrs.push(result.ssjph[i].submit_time);
            ...
            if (page != "") {
                ...
                for (var i = 0; i < result.pdddinfor.length; i++) {
                    applyHtml += "<tr>";
                    applyHtml += "<td>" + (i + 1) + "</td>";
                    applyHtml += "<td class='shqshjResourceBM'>" + result.ssjph[i].item_name + "</td>"; // 事项名称
                    applyHtml += "<td>" + result.ssjph[i].type + "</td>"; // 事项类型
                    applyHtml += "<td>" + result.ssjph[i].region_name + "</td>"; // 申请区域
                    applyHtml += "<td>" + result.ssjph[i].applicant + "</td>"; // 办理状态
                    applyHtml += "<td>" + result.ssjph[i].submit_time + "</td>"; // 收件时间
                    applyHtml += "</tr>";
                }
                // htmlElement =applyHtml;
                $("#qxzyqkTable tbody").html(applyHtml);
            }
        },
        error: function (error) {
        }
...
// 今日办结列表：Tab切换
$("#brbjProvide_propTab ul li").click(function () {
    $(this).addClass('active').siblings().removeClass('active');
    var date = new Date()
    if ($(this).html() == '当天') {
        day = dateFormat("YYYY-mm-dd", date)
...
/*今日办结列表*/
function getbrbjlb(day, mounth, year, page) {
    var brbj = ctx + "/tyslcontroller/getbrbjlb";
    var arr = new Array();
    $.ajax({
        type: "get",
        url: brbj,
        ...
        success: function (result) {
            ...
            var arrs = new Array();
            var arrss = new Array();
            var arrsss = new Array();
            for (var i = 0; i < result.brbj.length; i++) {
                arr.push(result.brbj[i].ITEM_NAME);
                arrs.push(result.brbj[i].SUBMIT_TIME)
                arrss.push(result.brbj[i].STATE);
                arrsss.push(result.brbj[i].WINDOW_SHOW_CODE)
            }
            ;
            // 获取图表 如果page是空的，说明要获取全部的部门数量。如果不是空的默认展示15条数据。
            if (page != "") {
                ...
                for (var i = 0; i < result.brbj.length; i++) {
                    applyHtml += "<tr>";
                    applyHtml += "<td>" + (i + 1) + "</td>";
                    applyHtml += "<td class='shqshjResourceBM'>" + result.brbj[i].ITEM_NAME + "</td>";
                    applyHtml += "<td>" + result.brbj[i].SUBMIT_TIME + "</td>";
                    applyHtml += "<td class='shqshjResourceBM'>" + result.brbj[i].STATE + "</td>";
                    applyHtml += "<td>" + result.brbj[i].WINDOW_SHOW_CODE + "</td>";
                    applyHtml += "</tr>";
                }
                // htmlElement =applyHtml;
                $("#brbjProvideProp tbody").html(applyHtml);
            }
        },
        error: function (error) {
        }
...
// 今日受理列表：Tab切换
$("#brslProvide_propTab ul li").click(function () {
    $(this).addClass('active').siblings().removeClass('active');
    var date = new Date()
    if ($(this).html() == '当天') {
        day = dateFormat("YYYY-mm-dd", date)
...
/*今日受理列表*/
function getbrsllb(day, mounth, year, page) {
    var brsl = ctx + "/tyslcontroller/getbrsllb";
    var arr = new Array();
    $.ajax({
        type: "get",
        url: brsl,
        ...
        success: function (result) {
            ...
            var arrss = new Array();
            var arrsss = new Array();
            for (var i = 0; i < result.brsl.length; i++) {
                arr.push(result.brsl[i].ITEM_NAME);
                arrs.push(result.brsl[i].SUBMIT_TIME)
                arrss.push(result.brsl[i].STATE);
                arrsss.push(result.brsl[i].WINDOW_SHOW_CODE)
            }
            ;
            // 获取图表 如果page是空的，说明要获取全部的部门数量。如果不是空的默认展示15条数据。
            if (page != "") {
                ...
                for (var i = 0; i < result.brsl.length; i++) {
                    applyHtml += "<tr>";
                    applyHtml += "<td>" + (i + 1) + "</td>";
                    applyHtml += "<td class='shqshjResourceBM'>" + result.brsl[i].ITEM_NAME + "</td>";
                    applyHtml += "<td>" + result.brsl[i].SUBMIT_TIME + "</td>";
                    applyHtml += "<td class='shqshjResourceBM'>" + result.brsl[i].STATE + "</td>";
                    applyHtml += "<td>" + result.brsl[i].WINDOW_SHOW_CODE + "</td>";
                    applyHtml += "</tr>";
                }
                // htmlElement =applyHtml;
                $("#brslProvideProp tbody").html(applyHtml);
            }
        },
        error: function (error) {
        }
...
// 今日办结事项：下载
function jinribanjieDownload() {
    var target = $("#brbjProvide_propTab ul li[class='active']").attr("data-target");
    var date = new Date();
    switch (target) {
        case 'day':
            window.location.href = ctx + "/tyslcontroller/downloadbrbjlb?year=&month=&day=" + dateFormat("YYYY-mm-dd", date);
            break;
        case 'month':
            window.location.href = ctx + "/tyslcontroller/downloadbrbjlb?year=&month=" + dateFormat("YYYY-mm", date) + "&day=";
            break;
        case 'year':
            window.location.href = ctx + "/tyslcontroller/downloadbrbjlb?year=" + dateFormat("YYYY", date) + "&month=&day=";
            break;
    }
}
...
function jinrishouliDownload() {
    var target = $("#brslProvide_propTab ul li[class='active']").attr("data-target");
    var date = new Date();
    switch (target) {
        case 'day':
            window.location.href = ctx + "/tyslcontroller/downloadbrsllb?year=&month=&day=" + dateFormat("YYYY-mm-dd", date);
            break;
        case 'month':
            window.location.href = ctx + "/tyslcontroller/downloadbrsllb?year=&month=" + dateFormat("YYYY-mm", date) + "&day=";
            break;
        case 'year':
            window.location.href = ctx + "/tyslcontroller/downloadbrsllb?year=" + dateFormat("YYYY", date) + "&month=&day=";
            break;
    }
}
...
// 空闲窗口列表：Tab切换
$("#kxckProvide_propTab ul li").click(function () {
    $(this).addClass('active').siblings().removeClass('active');
    var date = new Date()
    if ($(this).html() == '当天') {
        day = dateFormat("YYYY-mm-dd", date)
...
/*空闲窗口列表*/
function getkxcklb(day, mounth, year, page) {
    var kxcklb = ctx + "/tyslcontroller/getkxcklb";
    var arr = new Array();
    $.ajax({
        type: "get",
        url: kxcklb,
        ...
        success: function (result) {
            ...
            for (var i = 0; i < result.kxcklb.length; i++) {
                arr.push(result.kxcklb[i].REGION_NAME);
                arrs.push(result.kxcklb[i].WINDOW_SHOW_CODE)
                arrss.push(result.kxcklb[i].SS);
            ...
            if (page != "") {
                ...
                for (var i = 0; i < result.kxcklb.length; i++) {
                    applyHtml += "<tr>";
                    applyHtml += "<td>" + (i + 1) + "</td>";
                    applyHtml += "<td class='shqshjResourceBM'>" + result.kxcklb[i].REGION_NAME + "</td>";
                    applyHtml += "<td>" + result.kxcklb[i].WINDOW_SHOW_CODE + "</td>";
                    applyHtml += "<td>" + result.kxcklb[i].SS + "</td>";

                    applyHtml += "</tr>";
                }
                // htmlElement =applyHtml;
                $("#kxckProvideProp tbody").html(applyHtml);
            }
        },
        error: function (error) {
        }
...
//空闲窗口切换--树状图
$("#xxResource_propTabs ul li").click(function () {
    var date = new Date()
    $(this).addClass('active').siblings().removeClass('active');
    if ($(this).html() == '当日') {
        day = dateFormat("YYYY-mm-dd", date)
...
// 获取空闲窗口数量，以及详细情况--树状图
function getckkxck(day, mounth, year, page) {
    var kxcksl = ctx + "/sxgl/getckkxck";
    var arr = new Array();
    $.ajax({
        type: "get",
        async: true,
        ...
        success: function (result) {
            console.log(result.kxcksl)
            var arr = new Array();
            var arrs = new Array();
            for (var i = 0; i < result.kxcksl.length; i++) {
                arr.push(result.kxcksl[i].CODE);
                arrs.push({
                    "name": result.kxcksl[i].CODE,
                    "value": result.kxcksl[i].SUM
                })
            }
            ;
            var kxcksl = echarts.init(document.getElementById("kxcksl"));
            kxcksl.setOption({
                tooltip: {
                    trigger: "axis"
                },
...
// 空闲窗口办件数量：下载--树状图
function kxchuangkouDownload() {
    var target = $("#xxResource_propTabs ul li[class='active']").attr("data-target");
    var date = new Date();
    switch (target) {
        case 'day':
            window.location.href = ctx + "/sxgl/downloadkxck?year=&month=&day=" + dateFormat("YYYY-mm-dd", date);
            break;
        case 'month':
            window.location.href = ctx + "/sxgl/downloadkxck?year=&month=" + dateFormat("YYYY-mm", date) + "&day=";
            break;
        case 'year':
            window.location.href = ctx + "/sxgl/downloadkxck?year=" + dateFormat("YYYY", date) + "&month=&day=";
            break;
    }
}
